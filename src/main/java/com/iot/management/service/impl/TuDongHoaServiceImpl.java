...
import com.iot.management.repository.LichSuCanhBaoRepository;
import com.iot.management.repository.LichTrinhRepository;
import com.iot.management.repository.LuatNguongRepository;
...
public class TuDongHoaServiceImpl implements TuDongHoaService {
    ...
    // Theo dõi thời điểm điều kiện của luật bắt đầu được thỏa mãn để hỗ trợ "thời
    // gian duy trì điều kiện"
        ...
        try {
            ...
            for (String field : idents) {
                ...
                com.iot.management.model.entity.NhatKyDuLieu latest = nhatKyDuLieuRepository
                        .findTop1ByThietBi_MaThietBiAndTenTruongIgnoreCaseOrderByThoiGianDesc(targetDeviceId,
                                actualField);

                if (latest != null) {
                    Object value = null;
                    if (latest.getGiaTriSo() != null)
                        value = latest.getGiaTriSo();
                    else if (latest.getGiaTriLogic() != null)
                        value = latest.getGiaTriLogic();
                    else if (latest.getGiaTriChuoi() != null)
                        value = latest.getGiaTriChuoi();
            ...
            if (dataLog.getTenTruong() != null) {
                ...
                if (dataLog.getGiaTriSo() != null)
                    value = dataLog.getGiaTriSo();
                else if (dataLog.getGiaTriLogic() != null)
                    value = dataLog.getGiaTriLogic();
                else if (dataLog.getGiaTriChuoi() != null)
                    value = dataLog.getGiaTriChuoi();
    ...
    private void triggerAction(LuatNguong rule) {
        ...
        if (shouldLog) {
            ...
            try {
                ...
                logger.info(
                        "⚡ Luật tự động [ID: {}] kích hoạt: {} thiết bị '{}' (ID: {}) | Biểu thức: '{}' | Thời gian: {}",
                        rule.getMaLuat(),
                        actionText,
                        rule.getThietBi().getTenThietBi(),
                        rule.getThietBi().getMaThietBi(),
    ...
    // Trích xuất danh sách identifier (tên trường) từ biểu thức để lấy giá trị mới
    // nhất
