...
public class ThanhVienDuAnController {
    ...
    public ResponseEntity<?> capNhatVaiTro(@PathVariable("duAnId") Long duAnId,
        ...
        try {
            ...
            // Nếu nâng lên QUAN_LY, xóa các phân quyền chi tiết (không cần nữa vì có full
            // access)
            if (vaiTroMoi == DuAnRole.QUAN_LY) {
                ...
                List<PhanQuyenThietBi> devicePerms = phanQuyenThietBiRepository.findByMaNguoiDungAndMaDuAn(userId,
                        duAnId);
    ...
    public String chapNhanLoiMoi(@RequestParam("token") String token,
        ...
        try {
            ...
            model.addAttribute("message", "Đã chấp nhận lời mời thành công! Bạn đã trở thành thành viên của dự án "
    ...
    public ResponseEntity<Map<String, Object>> getDetailPermissions(
        ...
        try {
            ...
            for (KhuVuc area : allAreas) {
                ...
                // Nếu user có quyền QUAN_LY hoặc CHU_SO_HUU ở cấp dự án, tự động có quyền tất
                // cả khu vực
                if (hasFullAccess) {
            ...
            List<PhanQuyenThietBi> userDevicePerms = phanQuyenThietBiRepository.findByMaNguoiDungAndMaDuAn(userId,
                    duAnId);
        ...
        try {
            ...
            if (!duAnAuthService.laQuanLyTroLen(duAn.getMaDuAn(), currentUser.getMaNguoiDung())) {
                return ResponseEntity.status(403)
                        .body(Map.of("success", false, "message", "Bạn không có quyền quản lý dự án này"));
            }

            NguoiDung targetUser = nguoiDungRepository.findById(userId)
                    .orElseThrow(() -> new RuntimeException("Không tìm thấy người dùng"));

            // Kiểm tra vai trò của target user - chỉ cho phép phân quyền chi tiết cho
            // NGUOI_DUNG
            ...
            if (targetUserRole == DuAnRole.CHU_SO_HUU || targetUserRole == DuAnRole.QUAN_LY) {
                return ResponseEntity.status(400).body(Map.of("success", false, "message",
                        "Không thể phân quyền chi tiết cho CHỦ SỞ HỮU hoặc QUẢN LÝ"));
            }
    ...
    static class DetailPermissionsRequest {
        ...
        public List<AreaPermissionDTO> getAreaPermissions() {
            return areaPermissions;
        }

        public void setAreaPermissions(List<AreaPermissionDTO> areaPermissions) {
            this.areaPermissions = areaPermissions;
        }

        public List<DevicePermissionDTO> getDevicePermissions() {
            return devicePermissions;
        }

        public void setDevicePermissions(List<DevicePermissionDTO> devicePermissions) {
