    ...
    public ResponseEntity<?> getAlertStats(Authentication authentication) {
        ...
        Map<String, Long> byLevel = allAlerts.stream()
                .collect(Collectors.groupingBy(
                        a -> {
                            ...
                            if (noiDung == null) return "INFO";
                            if (noiDung.contains("nguy hiểm") || noiDung.contains("DANGER")) return "DANGER";
                            if (noiDung.contains("cảnh báo") || noiDung.contains("WARNING")) return "WARNING";
                            return "INFO";
                        },
                        Collectors.counting()
                ));
        ...
        List<Map<String, Object>> latestAlerts = allAlerts.stream()
                ...
                .map(alert -> {
                    ...
                    String level = "INFO";
                    if (noiDung != null) {
                        if (noiDung.contains("nguy hiểm") || noiDung.contains("DANGER")) level = "DANGER";
                        else if (noiDung.contains("cảnh báo") || noiDung.contains("WARNING")) level = "WARNING";
                    }
    ...
    public ResponseEntity<?> getScheduleStats(Authentication authentication) {
        ...
        Map<String, Long> byFrequency = schedules.stream()
                .collect(Collectors.groupingBy(
                        s -> {
                            ...
                            if (ngayTrongTuan == null || ngayTrongTuan.equals("*")) return "DAILY";
        ...
        List<Map<String, Object>> recentCommands = commands.stream()
                .map(cmd -> {
                    ...
                    cmdData.put("command", cmd.getTenLenh() + (cmd.getGiaTriLenh() != null ? ": " + cmd.getGiaTriLenh() : ""));
    ...
    public ResponseEntity<?> getFinanceStats(Authentication authentication) {
        ...
        List<Map<String, Object>> recentPayments = payments.stream()
                ...
                .map(payment -> {
                    ...
                    paymentData.put("package", payment.getDangKyGoi() != null && payment.getDangKyGoi().getGoiCuoc() != null 
                            ? payment.getDangKyGoi().getGoiCuoc().getTenGoi() : "N/A");
                    return paymentData;
                })
                .collect(Collectors.toList());
